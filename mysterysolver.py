...
def getNewData():
    #idfk get the image
    inputImage = cv.imread("IMG_1907.png",0)

    qmark = cv.imread("questionmark.png")
    vialtop = cv.imread("vial.png")

    img_rgb = inputImage
    img_rgb = cv.resize(img_rgb, totalImageSize)

    w, h = vialtop.shape[::-1]
    res = cv.matchTemplate(img_rgb, vialtop, cv.TM_CCOEFF_NORMED)
    threshold = 0.9
